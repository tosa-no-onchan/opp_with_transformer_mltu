...
'''
asr_with_transformer_mltu/transformer_asr_model.py
  transformer_asr_model.py
...
class SpeechFeatureEmbedding(layers.Layer):
    ...
    def build(self, input_shape):
        self.conv1 = tf.keras.layers.Conv1D(
             ...
             #self.num_hid, 3, strides=2, padding="same", activation="relu"        # changed by nishi 2023.8.10
        )
        self.conv2 = tf.keras.layers.Conv1D(
            self.num_hid, 11, strides=2, padding="same", activation="relu"
            #self.num_hid, 3, strides=2, padding="same", activation="relu"        # changed by nishi 2023.8.10
...
class Transformer(keras.Model):
    ...
    # add by nishi
    if False:
        def build_my(self,input_shape):
            print('>>> called Transformer:build()')
            super().build(input_shape)

            self.loss_metric = keras.metrics.Mean(name="loss")

            self.enc_input = SpeechFeatureEmbedding(num_hid=self.num_hid, maxlen=self.source_maxlen)
            self.dec_input = TokenEmbedding(
                num_vocab=self.num_classes, maxlen=self.target_maxlen, num_hid=self.num_hid
            )
            ...
            for i in range(self.num_layers_dec):
                setattr(
                    self,
                    f"dec_layer_{i}",
                    TransformerDecoder(self.num_hid, self.num_head, self.num_feed_forward),
                )
    ...
    def call(self, inputs):
        ...
        # type(source): <class 'tensorflow.python.framework.ops.SymbolicTensor'>
        #print('source.dtype:',source.dtype)
        # source.dtype: <dtype: 'float32'>
        #print('target.shape:',target.shape)
        # target.shape: (None, 199)
    ...
    #def build_from_config(self, config):
    #    print('>>> called Transformer:build_from_config()')
    #    super().build_from_config(config)
    #    self.classifier = config['classifier']
    def build(self,input_shape):
        self.classifier = layers.Dense(self.num_classes)
    ...
    def train_step(self, batch):
        ...
        with tf.GradientTape() as tape:
            ...
            # changed by nishi 2024.102.
            #loss = self.compute_loss(one_hot, preds, sample_weight=mask)
            # /home/nishi/kivy_env/lib/python3.10/site-packages/keras/src/backend/tensorflow/trainer.py:603: 
            # UserWarning: `model.compiled_loss()` is deprecated. 
    ...
    def generate(self, source, target_start_token_idx):
        ...
        # Expected shape (None, 1392, 193), but input has incompatible shape (1392, 193)
